...
# ╔═╡ 7929aba4-2a7d-4a2a-97ce-eeb1c4e6d53c
md"""
When $h$ is too small, the tangent line becomes inaccurate due to the loss of accuracy in floating point calculations.
"""
...
function euler_integrate(fprime::Function, fa::Number, 
	...
	for i in T
		push!(f, euler_integrate_step(fprime, fa, i, h))
		fa = f[end]
	end
...
Yes. Almost all people get infected, then recover. Not everybody.
"""
...
# ╔═╡ 7c59d039-cc85-42a6-ac70-bfee5ffd6d9e
@bind my_β Slider(0.01:0.01:1, show_value=true)

# ╔═╡ 8a6bcfa7-3c5b-42f8-9e5c-d6e231797680
@bind my_γ Slider(0.01:0.01:1, show_value=true)

# ╔═╡ 68274534-1103-11eb-0d62-f1acb57721bc
my_sir_results = euler_SIR(my_β, my_γ, 
	[0.99, 0.01, 0.00], 
	sir_T)

# ╔═╡ 79b52ac4-e645-4a4e-9d9e-e52c0727ef32
plot_sir!(plot(), sir_T, my_sir_results)

# ╔═╡ aee13dd9-482c-472f-883b-86fa91c82fc4
md"""
For an outbreak to occur, $\beta$ should be large enough, while $\gamma$ cannot be too large.
"""
...
Stop early when the absolute value of the gradient becomes lower than a certain threshold.
"""
...
function gradient_descent_2d_step(f, x0, y0; η=0.01)
	∂x, ∂y = gradient(f, x0, y0)
	return (x0 - ∂x * η, y0 - ∂y * η)
end
...
md"""
Yes. Different minima can be found by varying the starting point.
"""
...
found_μ, found_σ = let
	gradient_descent_2d(loss_dice, 30.0, 1.0; η = 0.5, N_steps = 10_000)
end
...
function loss_sir(β, γ)
	est_results = euler_SIR(β, γ, [0.99, 0.01, 0.00], spatial_T)
	return sum((spatial_result[1] - est_result[1]) ^ 2 for (spatial_result, est_result) in zip(spatial_results, est_results))
end
...
found_β, found_γ = let
	gradient_descent_2d(loss_sir, 0.02, 0.003; η = 1e-7, N_steps = 30)
end
