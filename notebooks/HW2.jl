...
# ╔═╡ 9acfefae-c459-4542-b7c0-525b036c00fc
colored_line(my_v)
...
function extend(M::AbstractMatrix, i, j)
	n, m = size(M)
	return M[min(n, max(1, i)), min(m, max(1, j))]
end
...
function box_blur(v::AbstractArray, l)
	...
	return [mean(extend(v, i + d) for d in -l:l) for i in eachindex(v)]
end
...
# ╔═╡ 36437c48-d0b1-4984-b46e-51ca681713c3
colored_line(box_blur(my_v, l_box))
...
function convolve(v::AbstractVector, k)
	n = length(v)
	l = length(k) ÷ 2
	return [sum(extend(v, i - m) * k[l + m + 1] for m in -l:l) for i in eachindex(v)]
end
...
function convolve(M::AbstractMatrix, K::AbstractMatrix)
	...
	return [sum(extend(M, i - p, j - q) * K[k + p + 1, l + q + 1] for p in -k:k for q in -l:l) for i in 1:n, j in 1:m]
end
...
function with_gaussian_blur(image; σ=3, l=5)
	g = [gauss(i, j; σ=σ) for i in -l:l, j in -l:l]
	g ./= sum(g)
	return convolve(image, g)
end

# ╔═╡ 31efc85f-836a-4378-90d0-866245bcf58a
...
# ╔═╡ 71e0d53c-1478-4a27-a2f6-a60bbfd6283b
with_sobel_edge_detect(philip_head)
