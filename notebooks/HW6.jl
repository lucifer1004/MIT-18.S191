...
mean_recovery_time = sum(large_experiment) / length(large_experiment)
...
let
	xs, ps = probability_distribution(large_experiment)
	bar(xs, ps, alpha=0.5, leg=false)	
	vline!([mean_recovery_time], ls=:dash)
end
...
let
	xs, ps = probability_distribution(large_experiment)
	bar(xs, log10.(ps), alpha=0.5, leg=false)	
	vline!([mean_recovery_time], ls=:dash)
end
...
@bind p_value Slider(0.0 : 0.05: 1.0)

# ╔═╡ 9a92eba4-ad68-4c53-a242-734718aeb3f1
@bind N_value Slider(0 : 5000 : 100000)

# ╔═╡ 562202be-5eac-46a4-9542-e6593bc39ff9
let
	new_experiment = experiment(p_value, N_value)
	xs, ps = probability_distribution(new_experiment)
	bar(xs, log10.(ps), alpha=0.5, leg=false)	
	vline!([mean_recovery_time], ls=:dash)
end
...
function τ(p, N = 10000)
	new_experiment = experiment(p, N)
	return sum(new_experiment) / N
end
...
```math
\sum_{k=1}^{\infty} P_k = p\left[1 + (1-p) + (1-p)^2 + \cdots \right] = 1
...
let
	...
	xs, ps = probability_distribution(new_experiment)
	plot(xs, ps)	
	plot!(xs, (x -> p_value * (1 - p_value) ^ (x - 1)).(xs))
end
...
md"""
```math
n(r,p) = \lfloor\frac{\log(1-r)}{\log(1-p)}\rfloor
```
"""
...
function experiment_fast(p::Real, N::Integer)
	
	return [geometric_fast(p) for i=1:N]
end
...
let
	p = 1e-10
	N = 10000
	new_experiment = experiment_fast(p, N)
	xs, ps = probability_distribution(new_experiment)
	bar(xs, log10.(ps), alpha=0.5, leg=false)
end
...
function atmosphere_experiment(p::Real, y0::Real, N::Integer, k::Integer)
	return [atmosphere(p, y0, N) for _ in 1:k]
end
...
let
	...
	N = 10_000_000
	k = 1000
	new_experiment = atmosphere_experiment(p, y0, N, k)
	xs, ps = probability_distribution(new_experiment)
	bar(xs, ps, alpha=0.5, leg=false)
end
...
@bind simulation_time Slider(0 : 10000 : 1000000)
...
let
	...
	y0 = 10
	k = 1000
	new_experiment = atmosphere_experiment(p, y0, simulation_time, k)
	xs, ps = probability_distribution(new_experiment)
	bar(xs, ps, alpha=0.5, leg=false)
end
...
# ╔═╡ 101deb1f-d89c-41d8-8b76-05cda906625e
let
	p = 0.55
	y0 = 10
	k = 1000
...
# ╔═╡ ec6db983-52e1-47ad-a3b9-b26777636fc8
let
	p = 0.51
	y0 = 10
	k = 10000
	new_experiment = atmosphere_experiment(p, y0, simulation_time, k)
	xs, ps = probability_distribution(new_experiment)
	bar(xs, log.(ps), alpha=0.5, leg=false)
end
...
```math
\begin{aligned}
p &=p_{0} \cdot\left(1-\frac{L \cdot h}{T_{0}}\right)^{\frac{g \cdot M}{R_{0} \cdot L}} \\
&=p_{0} \cdot\left(1-\frac{g \cdot h}{c_{\mathrm{p}} \cdot T_{0}}\right)^{\frac{c_{\mathrm{p}} \cdot M}{R_{0}}} \approx p_{0} \cdot \exp \left(-\frac{g \cdot h \cdot M}{T_{0} \cdot R_{0}}\right)
\end{aligned}
```
"""
