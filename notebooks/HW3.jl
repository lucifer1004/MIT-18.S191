...
unused_letters = [alphabet[i] for i in eachindex(alphabet) if sample_freqs[i] == 0] # replace with your answer
...
double_letters = [ch for ch in alphabet if sample_freq_matrix[index_of_letter(ch), index_of_letter(ch)] > 0]
...
row_sums = [sum(sample_freq_matrix[i, :]) for i in 1:size(sample_freq_matrix)[1]]

# ╔═╡ 4a0314a6-7dc0-4ee9-842b-3f9bd4d61fb1
col_sums = [sum(sample_freq_matrix[:, i]) for i in 1:size(sample_freq_matrix)[2]]
...
function word_counts(words::Vector)
	...
	for word in words
		counts[word] = word ∈ keys(counts) ? counts[word] + 1 : 1
	end
