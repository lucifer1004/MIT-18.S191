...
# ╔═╡ 97fef5df-e7c7-4d3e-9957-28fed25dfb88
f = x -> x * (1 / x)

# ╔═╡ ea28bf57-ba62-41ce-8be6-d38ca2c5caa3
not_so_exact_numbers = [j for j=1:1000 if f(j) ≠ 1]

# ╔═╡ 8efcaaeb-4900-404b-ae59-65db0bde8790
error_dist = abs(f(not_so_exact_numbers[1]) - 1)
...
RankTwoMatrix(RankOneMatrix(1.0:1.0:10.0), RankOneMatrix(0.0:0.1:0.9))
...
function Base.getindex(M::LowRankMatrix, i, j)
	...
	return sum(x[i, j] for x in M.Ms)
end
...
function print_as_matrix(M::FirstRankOneMatrix)
	n, m = size(M)
	for i in 1:n
		println(join([rpad(string(M[i, j]), 6) for j in 1:m], ""))
	end
end
...
From rank $\lceil\frac{n}{2}\rceil$,

Because we will need $2\lceil\frac{n}{2}\rceil$ vectors of $n$ vertices.
"""
...
_, singular_values_of_biggie, _ = svd(biggie)
...
function k_rank_ones(k, m, n)
	return sum(RankOneMatrix(rand(m), rand(n)) for _ in 1:k)
end
...
# ╔═╡ 49232de3-7f3f-4ac7-83ed-504a339439d3
numerical_rank(k_rank_ones(1, 3, 3))

# ╔═╡ 4079e66f-5e75-4220-bd89-1d41d93a3d19
numerical_rank(k_rank_ones(2, 3, 3))

# ╔═╡ fc9df490-5a0c-4603-a513-7febac2414e4
numerical_rank(k_rank_ones(3, 3, 3))

# ╔═╡ bf360099-268e-48e9-b0c0-1d8b35f1ce6c
numerical_rank(k_rank_ones(4, 3, 3))

# ╔═╡ a6bb92f9-0cb2-4fdd-8b67-f286edbbdcb6
numerical_rank(k_rank_ones(4, 5, 2))

# ╔═╡ 57c19601-122b-414f-bc99-56f98c794e61
numerical_rank(k_rank_ones(4, 1, 5))
