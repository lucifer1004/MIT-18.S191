...
function greedy_seam(energies, starting_pixel::Int)
	...
	seam = [a]
	for row in 2:m
		a = argmin(x -> energies[row, x], max(1, a - 1):min(a + 1, n))
		push!(seam, a)
	end
	seam
end
...
function least_energy(energies, i, j)
	...
	nxt = [(least_energy(energies, i + 1, x)[1], x) for x in max(1, j - 1):min(n, j + 1)]

	return (energies[i, j] + minimum(nxt)[1], minimum(nxt)[2])
end
...
When given a starting pixel, the number of possible seams grows in $\mathcal{O}(3^m)$, regardless of $n$.
"""
...
function memoized_least_energy(energies, i, j, memory::Dict)
	...
	nxt = [(memoized_least_energy(energies, i + 1, x, memory)[1], x) for x in max(1, j - 1):min(n, j + 1)]

	result = (energies[i, j] + minimum(nxt)[1], minimum(nxt)[2])
	memory[(i, j)] = result
	return result
end
...
function memoized_recursive_seam(energies, starting_pixel)
	...
	seam = [a]
	for i in 1:m - 1
		a = memoized_least_energy(energies, i, a, memory)[2]
		push!(seam, a)
	end
	seam
end
...
function matrix_memoized_least_energy(energies, i, j, memory::Matrix)
	...
	nxt = [(matrix_memoized_least_energy(energies, i + 1, x, memory)[1], x) for x in max(1, j - 1):min(n, j + 1)]
...
function matrix_memoized_seam(energies, starting_pixel)
	...
	seam = [a]
	for i in 1:m - 1
		a = matrix_memoized_least_energy(energies, i, a, memory)[2]
		push!(seam, a)
	end
	seam
end
...
function least_energy_matrix(energies)
	...
	for i in m-1:-1:1
		for j in 1:n
			result[i, j] += minimum([result[i + 1, x] for x in max(1, j - 1):min(n, j + 1)])
		end
	end
...
function seam_from_precomputed_least_energy(energies, starting_pixel::Int)
	...
	for i in 2:m
		a = argmin(x -> energies[i, x], max(1, a - 1):min(n, a + 1))
		push!(ans, a)
	end
