...
The smaller the value of $B$, the slower the temperature increases.
"""
...
The temperature increases faster than linear increase.
"""
...
ECS_samples = let
	...
	samples = rand(B_distribution, Nsamples)
	# we only sample negative values of B
	map(b -> ECS(B=b), filter(x -> x < 0, samples))
end
...
histogram(ECS_samples, size=(600, 300), label=nothing, xlabel="ECS", ylabel="samples")
...
# ╔═╡ c65e7d05-02b1-4a72-997e-1f110da1ea4b
ECS(B = mean(B_samples))

# ╔═╡ aac95027-853c-4a8e-a8ec-30332ef58cbd
count(x -> x > ECS(B = mean(B_samples)), ECS_samples) / length(ECS_samples)
...
function temperature_response(CO2::Function, B::Float64=-1.3)
	model = Model.EBM(14.0, 1850, 1, CO2; B=B)
	Model.run!(model, 2100)
	return model.T[end]
end
...
expected_double_CO2_year = let
	t[findfirst(x -> Model.CO2_RCP85(x) >= 2 * Model.CO2_RCP85(t[1]), t)]
end
...
# ╔═╡ 12c76d13-3bf7-4f83-b273-125224097bae
function temperature_response(T0::Float64, CO2::Function, B::Float64=-1.3)
	model = Model.EBM(T0, 1850, 1, CO2; B=B)
	Model.run!(model, 2100)
	return model.T[end]
end
...
let
	responses = (x -> temperature_response(Model.CO2_RCP85, x)).(B_samples)
	histogram(responses, title="RCP8.5")
end

# ╔═╡ 85d8003d-2ef8-467f-8437-76d7356598a6
let
	responses = (x -> temperature_response(Model.CO2_RCP26, x)).(B_samples)
	histogram(responses, title="RCP2.6")
end
...
@bind log_CO2 Slider(1:0.0001:6, default=log10(280))

# ╔═╡ 70e82c73-ca6b-4402-ba87-fbdd39f7faad
CO2 = 10.0 ^ log_CO2
...
let
	...
	my_CO2 = 10 .^ (1.0:0.01:6.0)
	my_T = map(CO2 -> temperature_response(14.0, t -> CO2), my_CO2)
	plot!(p, my_CO2, my_T, alpha=0.3, c=:red, label = nothing)

	my_CO2 = 10 .^ (1.0:0.01:6.0)
	my_T = map(CO2 -> temperature_response(-38.3, t -> CO2), my_CO2)
	plot!(p, my_CO2, my_T, c=:lightblue, label = nothing)

	add_reference_points!(p)

	new_ebm = step_model!(ebm, CO2)
	
	plot!(p, 
		[new_ebm.CO2(new_ebm.t[end])], [new_ebm.T[end]],
		label=nothing,
		color=:black,
		shape=:circle,
	)
...
# ╔═╡ ba44e779-d9e0-4a52-bd2a-c4371df6ce6c
equilibrium_temperature(CO2) = temperature_response(-48.0, x -> CO2)
...
co2_to_melt_snowball = let
	...
	while hi - lo > 1e-8
		mid = (lo + hi) / 2
		if equilibrium_temperature(mid) > 10
			hi = mid
		else
			lo = mid
		end
