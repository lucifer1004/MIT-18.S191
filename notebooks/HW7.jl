...
student = (name = "Gabriel Wu", kerberos_id = "gabriel_wu")
...
# ╔═╡ ae3ccc48-0e48-4ecf-a272-913b0634bd70
...
function sir_mean_plot(simulations::Vector{<:NamedTuple})
	...
	S_mean = sum(simulation.S for simulation in simulations) / N
	I_mean = sum(simulation.I for simulation in simulations) / N
	R_mean = sum(simulation.R for simulation in simulations) / N
	p = plot()
	plot!(p, 1:T, S_mean, alpha=.5, label="S")
	plot!(p, 1:T, I_mean, alpha=.5, label="I")
	plot!(p, 1:T, R_mean, alpha=.5, label="R")
	return p
end
...
# ╔═╡ 5f605b92-4bab-4268-8068-082805d3860a
@bind N Slider(2:200, show_value=true)

# ╔═╡ 58a67903-4948-4c25-9f77-06501e2c85f2
@bind T Slider(100:100:2000, show_value=true)

# ╔═╡ 1c6aa208-04d1-11eb-0b87-cf429e6ff6d0
@bind p_infection Slider(0.0:0.001:1.0, show_value=true)

# ╔═╡ 0919e837-fb3f-44c0-9899-1947f4f4fe48
@bind p_recovery Slider(0.0:0.001:0.01, show_value=true)

# ╔═╡ a05a749d-dfe1-4853-ac6c-bde0cb3ee069
@bind num_simulations Slider(1:500, show_value=true)
...
function sir_mean_error_plot(simulations::Vector{<:NamedTuple})
	...
	S_std = [std(simulation.S[i] for simulation in simulations) for i in 1:T]
	I_std = [std(simulation.I[i] for simulation in simulations) for i in 1:T]
	R_std = [std(simulation.R[i] for simulation in simulations) for i in 1:T]
	p = plot()
	plot!(p, 1:T, S_mean, yerror=S_std, alpha=.5, label="S")
	plot!(p, 1:T, I_mean, yerror=I_std, alpha=.5, label="I")
	plot!(p, 1:T, R_mean, yerror=R_std, alpha=.5, label="R")
	return p
end
...
# ╔═╡ c0e0b437-1169-46f6-be9a-c560cd17f9fb
begin
	# Helper functions from HW6

	function counts(data::Vector)
		counts = Dict{Int,Int}()
		
		for num in data
			counts[num] = num ∈ keys(counts) ? counts[num] + 1 : 1
		end	
...
function step!(agents::Vector{Agent}, infection::AbstractInfection)
	...
	a = rand(1:N)
	b = rand(1:N)
	interact!(agents[a], agents[b], infection)
	return agents
end
...
function simulation(N::Integer, T::Integer, infection::AbstractInfection)
	...
	for i in 1:T
		...
		push!(S_counts, count(is_susceptible, agents))
		push!(I_counts, count(is_infected, agents))
		push!(R_counts, count(is_recovered, agents))
	end
...
function repeat_simulations(N = 100, T = 1000, infection = InfectionRecovery(0.02, 0.002), num_simulations = 20)
...
# ╔═╡ c14c83ef-2731-4f96-9b4c-f0b86e09c7f4
new_simulations = repeat_simulations(N, T, InfectionRecovery(p_infection, p_recovery), num_simulations)

# ╔═╡ 2a71dc6f-a529-4662-b4f9-977a65825635
sir_mean_error_plot(new_simulations)

# ╔═╡ 26e2978e-0435-11eb-0d61-25f552d2771e
let
	p = plot()
	for t in 0.2:0.2:1.0
		pd = probability_distribution([simulation.I[Int(T * t)] for simulation in new_simulations])
		bar!(p, pd, alpha=0.5, label="T = $(Int(T * t))")
	end
	p
end
...
let
	...
	for simulation in simulations
		plot!(1:T, simulation.I, alpha=0.5, label=nothing)
	end
	plot!(1:T, I_mean, lw=5, ls=:dash, label="Mean")
end
...
md"""
In the end, $I$ and $S$ will come to a dynamic balance.
"""
