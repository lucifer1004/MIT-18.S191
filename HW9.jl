...
function trajectory(w::Coordinate, n::Int)
	...
	return accumulate(+, [w; moves...])[2:end]
end
...
function trajectory(c::Coordinate, n::Int, L::Number)
	moves = rand(possible_moves, n)
	return accumulate((x, y) -> collide_boundary(x + y, L), [c; moves...])[2:end]
end
...
# ╔═╡ fbae8ed7-3a4c-4ed4-9710-66d8b1a862ab
let
	# Create a new plot with aspect ratio 1:1
	p = plot(ratio=1)

	for i in 1:10
		plot_trajectory!(p, trajectory(Coordinate(5,5), 1000, 20))	
	end
...
# ╔═╡ e1b5bcb9-3b0f-4ad7-9a81-8bb221f2462f
...
begin
	...
	Agent(position, status) = Agent(position, status, 0)
end
...
function initialize(N::Number, L::Number)
	agents = [Agent(Coordinate(rand(-L:L), rand(-L:L)), S) for i in 1:N]
	infected = rand(1:N)
	agents[infected].status = I
	return agents
end
...
function interact!(agent::Agent, source::Agent, infection::CollisionInfectionRecovery)
	if agent.status == S && source.status == I && agent.position == source.position
...
causes_outbreak = CollisionInfectionRecovery(0.9, 1e-6)
...
People will only get infected when they spatially meet other people.
"""
...
function visualize(agents::Vector, L)
	p = plot(ratio=1.0)
	for agent in agents
		x, y = make_tuple(position(agent))
		scatter!(p, [x], [y], label=nothing, color=color(agent), alpha=0.5)
	end
	return p
end
...
function initialize_social(N, L)
	agents = [SocialAgent(Coordinate(rand(-2L:2L), rand(-2L:2L)), S, 0, rand(LinRange(0.1, 0.5, 10))) for i in 1:N]
	infected = rand(1:N)
	agents[infected].status = I
	return agents
end
...
function interact!(agent::SocialAgent, source::SocialAgent, infection::CollisionInfectionRecovery)
	if agent.status == S && source.status == I && agent.position == source.position
		if rand() < agent.social_score + source.social_score && rand() < infection.p_infection
			agent.status = I
			source.num_infected += 1
		end
	elseif agent.status == I
...
function step!(agents::Vector, L::Number, infection::AbstractInfection)
	N = length(agents)
	source_number = rand(1:N)
	agents[source_number].position = collide_boundary(agents[source_number].position + rand(possible_moves), L)
	for i in 1:N
		if i != source_number
			interact!(agents[i], agents[source_number], infection)
		end
	end
	return agents
end

# ╔═╡ 0f473754-1458-4ac3-a5b1-da9a83c21f85
...
let
	...
	for _ in 1:k_sweeps
		sweep!(agents, L, pandemic)
		s = count(x -> x.status == S, agents)
		i = count(x -> x.status == I, agents)
		r = count(x -> x.status == R, agents)
		push!(Sc, s)
		push!(Ic, i)
		push!(Rc, r)
	end
...
let
	...
	for rep in 1:50
		...
		plot!(p, Sc, color="blue", alpha=0.5, label=nothing)
		plot!(p, Ic, color="red", alpha=0.5, label=nothing)
		plot!(p, Rc, color="green", alpha=0.5, label=nothing)
	end
...
function visualize_dynamic(N = 50, L = 30, Tmax = 200, infection = pandemic)
	agents = initialize(N, L)
...
# ╔═╡ 78703b7e-30e5-41c7-95fd-939e1163d207
visualize_dynamic(100, 20, 200, causes_outbreak)

# ╔═╡ 4d4548fe-0a66-11eb-375a-9313dc6c423d
visualize_dynamic(100, 20, 200, does_not_cause_outbreak)
...
let
	...
	for _ in 1:20
		...
		for _ in 1:10000
			sweep!(agents, L, causes_outbreak)
		end
	
		scatter!(p, [agent.social_score for agent in agents], [agent.num_infected for agent in agents], label=nothing, alpha=0.1, color=:blue)
